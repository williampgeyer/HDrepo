...
# Chapter 5: Permits LAG ----------------------------
# Libraries and Functions -------------------------------------------------

library(lubridate)

# Data Retrieval ----------------------------------------------------------
dt.TTM.Permits = rbind(fread("https://www2.census.gov/econ/bps/County/co2309c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2308c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2307c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2306c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2305c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2304c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2303c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2302c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2301c.txt"), 
                       ...
                       fread("https://www2.census.gov/econ/bps/County/co2203c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2202c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2201c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2112c.txt"), 
                       fread("https://www2.census.gov/econ/bps/County/co2111c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2110c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2109c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2108c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2107c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2106c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2105c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2104c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2103c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2102c.txt"),
                       fread("https://www2.census.gov/econ/bps/County/co2101c.txt"))
...
#check work
dt.TTM.Permits[, list(Units = sum(SFH_Units)), by = Date] %>% copytoexcel()



# Permit Loop -------------------------------------------------------------

# create a copy to restore to before each loop
dt.permits.copy = copy(dt.TTM.Permits)

# run loop

dt.permits_lag_output = rbindlist(lapply(0:12,function(n.lag){
  date.end = as.Date("2023-09-01") - months(n.lag)
  date.start = date.end - years(1) + months(1)
  mo.end = paste0(substr(as.character(date.end),1,4),substr(as.character(date.end),6,7))
  mo.start = paste0(substr(as.character(date.start),1,4),substr(as.character(date.start),6,7))
  
  dt.TTM.Permits = dt.permits.copy
  
  dt.TTM.Permits[, time_frame := "exclude"]
  dt.TTM.Permits[Date  <= mo.end & Date >=  mo.start, time_frame := "March_23_TTM"]
  dt.TTM.Permits = dt.TTM.Permits[time_frame == "March_23_TTM"]
  ...
  dt.permit.market = dt.permit.market
  return(data.table(nlag=n.lag,dt.permit.market))
  
}))



# Chapter 6: Realtor.com LAG ----------------------------------------
# Data Retrieval ----------------------------------------------------------
...
# create a copy to restore to before each loop
dt.realtor.combined_copy = copy(dt.realtor.combined)

# run loop

dt.realtor_lag_output = rbindlist(lapply(0:12,function(n.lag){
  date.end = as.Date("2023-09-01") - months(n.lag)
  date.start = date.end - years(1) + months(1)
  mo.end = paste0(substr(as.character(date.end),1,4),substr(as.character(date.end),6,7))
  mo.start = paste0(substr(as.character(date.start),1,4),substr(as.character(date.start),6,7))
  
  dt.realtor.combined = dt.realtor.combined_copy
  
  # paste in realtor calc
  dt.realtor.combined[, time_frame := "exclude"]
  dt.realtor.combined[core_month_date_yyyymm  <= mo.end & core_month_date_yyyymm >=  mo.start, time_frame := "March_23_TTM"]
  ...
  return(data.table(nlag=n.lag,dt.realtor.results))
  
}))


# Chapter 7: Dealer Performance Summary ----------------------------------------------
...
dt.hd_perf = merge(x = dt.hd_perf, y = dt.level2, by.x = c("purchasezip"), by.y = c("mkt_purchasezip")) %>% 
  merge(y = dt.permits_lag_output[nlag == 5], by = c("purchasezip")) %>% #DEFINE LAG!!!!!!!!!
  merge(y = dt.realtor_lag_output[nlag ==3], by = c("purchasezip")) #DEFINE LAG!!!!!!!!! 

dt.hd_perf = dt.hd_perf[order(purchasezip)]

#add in volumes and customer ID

dt.ttm.units = data.table(read.csv("./TTM Units v3.csv"))
dt.ptm.units = data.table(read.csv("./PTM Units v3.csv"))

dt.ttm.units = dt.ttm.units[MG.Product...Level.1 == "Finished Product", list(MG.Booking.Amt = sum(Booking_Cleaned), 
                                   MG.Booking.Units = sum(Units_Cleaned)), by = list(MG.Customer.Level.3.ID, MG.Company...Level.3)]


dt.ptm.units = dt.ptm.units[MG.Product...Level.1 == "Finished Product", list(MG.Booking.Amt = sum(Booking_Cleaned), 
                                   MG.Booking.Units = sum(Units_Cleaned)), by = list(MG.Customer.Level.3.ID, MG.Company...Level.3)]


customer_ID = dt.hd2[, list(Customer_ID = customer_id3, storename)] %>% unique()

dt.hd_perf = merge(x = dt.hd_perf, y = customer_ID, by = c("storename")) %>% 
  merge(y = dt.ttm.units[, list(Customer_ID = MG.Customer.Level.3.ID, brand = MG.Company...Level.3, TTM_units = MG.Booking.Units)], by = c("Customer_ID", "brand")) %>% 
  merge(y = dt.ptm.units[, list(Customer_ID = MG.Customer.Level.3.ID, brand = MG.Company...Level.3, PTM_units = MG.Booking.Units)], by = c("Customer_ID", "brand")) 

#Calculate Volume by market

dt.vol.market.1 = dt.hd_perf[, list(TTM_units = sum(TTM_units), 
                                    PTM_units = sum(PTM_units), 
                                    booking_TTM = sum(`2023_TTM`)),
                             by = list(Market = purchasezip)]

dt.vol.market = merge(x = dt.vol.market.1, y = dt.dealer_mkt_mapping, by.x = c("Market"), by.y = c("mktzip"))

dt.vol.market = dt.vol.market[, list(mkt_TTM_units = sum(TTM_units), 
                                     mkt_PTM_units = sum(PTM_units)), by = purchasezip]

dt.vol.market.2 = dt.hd_perf[, list(TTM_units = sum(TTM_units), 
                                    PTM_units = sum(PTM_units), 
                                    booking_TTM = sum(`2023_TTM`)),
                             by = list(Market = purchasezip, brand)]

dt.vol.market.2 = merge(x = dt.vol.market.2, y = dt.dealer_mkt_mapping, by.x = c("Market"), by.y = c("mktzip"))

dt.vol.market.2 = dt.vol.market.2[, list(mkt_TTM_units = sum(TTM_units), 
                                     mkt_PTM_units = sum(PTM_units)), by = list(purchasezip, brand)]

dt.vol.market.2 = dcast.data.table(dt.vol.market.2, purchasezip ~ brand, value.var = "mkt_TTM_units")

dt.vol.market.2 = dt.vol.market.2[, list(mkt_CBG_Units = CBG, mkt_HD_Units = `HD Brand`), by = list(purchasezip)]


# Chapter 8: POE Definition -----------------------------------------------------

#subset to relevant metrics
dt.poenew = dt.hd_perf[,list(purchasezip,storename,alliance, alliance_num,brand, `2022_PTM`,`2023_TTM`,mkt_comp_dealers,mkt_HD_program_dealers,
                             mkt_Grouping,mkt_Own_perc,mkt_Median_census_home_value,mkt_Households, 
                             Pendings = RCOM_Pendings_SS_Daily, List_Price = RCOM_List_Price, Permits = mkt_Building_Permits_march_TTM, mkt_HD_bookingamt, 
                             TTM_units, PTM_units)]

#add in program dealers
dt.poenew[, Program_Dealer := "No"]
dt.poenew[(brand=="CBG" & alliance_num<8)|(brand=="HD Brand" & alliance_num<5), Program_Dealer := "Yes"]

#add in penetration
dt.poenew[, mkt_Bookings_per_HH := mkt_HD_bookingamt / mkt_Households]

#add in market Size 

dt.poenew[,est_market := mkt_HD_bookingamt/(mkt_HD_program_dealers/(mkt_HD_program_dealers+mkt_comp_dealers))]

#add in standard cost

dt.std.cost = copy(dt.hd2)[year == "2023_TTM"][, list(stdcost_23 = sum(stdcost, na.rm = T), 
                                                      msrp_23 = sum(msrp)), 
                                               by = list(storename, purchasezip, brand, customer_id3)] 

dt.poenew = merge(x = dt.poenew, y = dt.std.cost, by = c("storename", "purchasezip", "brand"), all.x = T)

#add in market volume

dt.poenew = merge(x = dt.poenew, y = dt.vol.market, by = c("purchasezip"))
dt.poenew = merge(x = dt.poenew, y = dt.vol.market.2, by = c("purchasezip"), all.x = T)

#note - there are dealers missing standard costs, but that is only 0 TTM 

#lets filter to postive ttm

dt.poenew = dt.poenew[`2023_TTM` > 0]


#Mulit-variate to predict Booking $s

#                             Estimate   Std. Error    t value      Pr(>|t|)
# (Intercept)            -2.958401e+01 1.462025e+00 -20.234947  1.313010e-87
# mkt_Own_perc            5.704675e+01 1.870089e+00  30.504829 9.603340e-188
# RCOM_List_Price_lagged  1.792864e-05 5.000559e-07  35.853283 4.974959e-251
# mkt_GroupingSub_Urban  -4.727927e+00 4.748467e-01  -9.956745  3.844885e-23
# mkt_GroupingUrban      -4.742350e+00 5.692398e-01  -8.331023  1.021827e-16

dt.poenew[,projected_spending := -2.958401e+01 + (mkt_Grouping=="Sub_Urban")*(-4.727927) + (mkt_Grouping=="Urban")*(-4.742350) + mkt_Own_perc*(5.704675e+01) + List_Price*(1.792864e-05)]

dt.poenew[,growth_perc := ecdf((Pendings+Permits)/mkt_Households)((Pendings+Permits)/mkt_Households), by = mkt_Grouping] #Percentile of Pendings + Permits / HHs
dt.poenew[,spending_perc := ecdf(projected_spending)(projected_spending), by = mkt_Grouping] #Percentile of projected spending
dt.poenew[,comp_perc := ecdf(mkt_comp_dealers/mkt_HD_program_dealers)(mkt_comp_dealers/mkt_HD_program_dealers)] #Percentile of of comp dealers / HD Dealers


dt.poenew[mkt_Grouping=="Rural" & growth_perc > 0.5 & spending_perc > 0.5,poe := "1"]
dt.poenew[mkt_Grouping=="Rural" & growth_perc > 0.5 & spending_perc <= 0.5, poe := "3"]
dt.poenew[mkt_Grouping=="Rural" & growth_perc <= 0.5 & spending_perc > 0.5, poe := "2"]
dt.poenew[mkt_Grouping=="Rural" & growth_perc <= 0.5 & spending_perc <= 0.5, poe := "4"]

dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc > 0.5 & spending_perc > 0.5 & comp_perc <= 0.5, poe := "5"]
dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc > 0.5 & spending_perc > 0.5 & comp_perc > 0.5, poe := "5"]

dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc > 0.5 & spending_perc <= 0.5 & comp_perc <= 0.5, poe := "7"]
dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc > 0.5 & spending_perc <= 0.5 & comp_perc > 0.5, poe := "7"]

dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc <= 0.5 & spending_perc > 0.5 & comp_perc <= 0.5, poe := "6"]
dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc <= 0.5 & spending_perc > 0.5 & comp_perc > 0.5, poe := "6"]

dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc <= 0.5 & spending_perc <= 0.5 & comp_perc <= 0.5, poe := "8"]
dt.poenew[mkt_Grouping=="Sub_Urban" & growth_perc <= 0.5 & spending_perc <= 0.5 & comp_perc > 0.5, poe :=  "8"]


dt.poenew[mkt_Grouping=="Urban" & growth_perc > 0.5 & spending_perc > 0.5 & comp_perc <= 0.5, poe := "9"]
dt.poenew[mkt_Grouping=="Urban" & growth_perc > 0.5 & spending_perc > 0.5 & comp_perc > 0.5, poe := "9"]

dt.poenew[mkt_Grouping=="Urban" & growth_perc > 0.5 & spending_perc <= 0.5 & comp_perc <= 0.5, poe := "11"]
dt.poenew[mkt_Grouping=="Urban" & growth_perc > 0.5 & spending_perc <= 0.5 & comp_perc > 0.5, poe := "11"]

dt.poenew[mkt_Grouping=="Urban" & growth_perc <= 0.5 & spending_perc > 0.5 & comp_perc <= 0.5, poe := "10"]
dt.poenew[mkt_Grouping=="Urban" & growth_perc <= 0.5 & spending_perc > 0.5 & comp_perc > 0.5, poe := "10"]

dt.poenew[mkt_Grouping=="Urban" & growth_perc <= 0.5 & spending_perc <= 0.5 & comp_perc <= 0.5, poe := "12"]
dt.poenew[mkt_Grouping=="Urban" & growth_perc <= 0.5 & spending_perc <= 0.5 & comp_perc > 0.5, poe :=  "12"]


#summarize POEs

dt.poenew[ brand == "CBG",list(Booking_23 = sum(`2023_TTM`),
                 Booking_22 = sum(`2022_PTM`),
                 Booking_Growth_Perc = round(100*(sum(`2023_TTM`)/sum(`2022_PTM`)-1),0),
                 HD_Margin = round((100*(sum(`2023_TTM`) - sum(stdcost_23)) / sum(`2023_TTM`, na.rm = T)), 0), 
                 Dealers_Total = .N, 
                 Dealers_Program = sum(Program_Dealer == "Yes"), 
                 Median_mkt_Penetration = round(median(mkt_Bookings_per_HH), 1), 
                 WA_mkt_Penetration = round(sum(mkt_HD_bookingamt)/sum(mkt_Households), 1), 
                 PD_Dealers_per_10k_HH = ((sum(Program_Dealer == "Yes") / sum(mkt_Households))*10000), 
                 Vol_Growth = round(100*(sum(TTM_units)/sum(PTM_units)-1),0), 
                 Vol_23 = sum(TTM_units), 
                 Vol_22 = sum(PTM_units), 
                 Vol_per_HH = sum(mkt_CBG_Units)/sum(mkt_Households)),
           by = list(poe)] %>% merge(dt.poenew[ ,list(Booking_23 = sum(`2023_TTM`)),
                                                by = list(poe, Program_Dealer)][order(poe, -Program_Dealer)] %>% 
                                       dcast.data.table(poe ~ Program_Dealer, value.var = "Booking_23") %>% .[, list(Non_PD_Booking = No, PD_Booking = Yes), by = poe], 
                                     by = c("poe")) %>% .[, Perc_PD_Booking := round((PD_Booking / Booking_23)*100, 0)] %>% .[, Perc_Booking := round((Booking_23 / sum(Booking_23))*100, 1)] %>%
  .[, Program_Bookings_per_PD := round((PD_Booking / Dealers_Program) / 1000, 0)] %>% 
  print() %>% copytoexcel()


dt.poenew[, list(WA_mkt_size = round(sum(est_market)/sum(mkt_Households), 1)), 
          by = poe] %>% copytoexcel()

dt.mk.dens.summary = copy(dt.poenew)[, list(purchasezip,
                                            mkt_HD_program_dealers, 
                                            mkt_Households,mkt_HD_bookingamt, poe)] %>% unique()

dt.mk.dens.summary[, list(PD_per_HH = (sum(mkt_HD_program_dealers) / sum(mkt_Households))*100000, 
                          Booking_per_HH = sum(mkt_HD_bookingamt) / sum(mkt_Households)),
                   by = poe][order(poe)] %>% copytoexcel()


dt.pg = dt.poenew[Program_Dealer == "Yes", list(storename, purchasezip, brand, poe, customer_id3, `2023_TTM`)]

dt.pg[order(storename, customer_id3 )]


# Exporting Script --------------------------------------------------------

write.csv(dt.realtor.results,"./purchase level list ttm.csv")

dt.henry = data.table(read.csv("./henry poe data.csv"))

dt.henry = merge(x = dt.henry, y = dt.poenew, by.x = c("customer_id", "brand"), by.y = c("customer_id3", "brand"))
